...
test("deve abrir detalhes do livro e perfil do usuario", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: /Recepção do catálogo/i })).toBeVisible({ timeout: 15000 });
  await expect(page.getByRole("heading", { name: /Continuar explorando/i })).toBeVisible({ timeout: 15000 });

  await page.getByRole("navigation", { name: "Navegação do usuário" }).getByRole("link", { name: "Perfil", exact: true }).click();
  await expect(page).toHaveURL(/\/profile$/);
  await expect(page.getByRole("heading", { name: "Perfil e histórico de leitura" })).toBeVisible({ timeout: 15000 });
  await expect(page.getByRole("heading", { name: "Ações rápidas" })).toBeVisible({ timeout: 15000 });
});
...
test("deve permitir revisar preferências e visualizar o ranking", async ({ page }) => {
  ...
  await expect(page.getByText("Preferências atualizadas com sucesso.")).toBeVisible();

  await page.getByRole("navigation", { name: "Navegação do usuário" }).getByRole("link", { name: "Ranking", exact: true }).click();
  ...
  await expect(
    page
      ...
      .or(page.getByText("Nenhum participante elegível nesta semana"))
  ).toBeVisible();
});
...
test("deve criar, editar e remover uma avaliação", async ({ page }) => {
  ...
  await page.getByRole("spinbutton", { name: "Página" }).fill("5");
  ...
  await page.getByRole("navigation", { name: "Navegação do usuário" }).getByRole("link", { name: "Avaliações", exact: true }).click();
  ...
  const createCard = page.locator("article.card").filter({
    has: page.getByRole("heading", { name: "Nova avaliação" }),
  }).first();
...
test("deve abrir conquistas e mostrar progresso", async ({ page }) => {
  ...
  await page.getByRole("navigation", { name: "Navegação do usuário" }).getByRole("link", { name: "Conquistas", exact: true }).click();
...
test("deve abrir o fluxo de recuperação de senha", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: /Recuperar senha|Definir nova senha/i })).toBeVisible();
...
test("deve salvar progresso de leitura e atualizar meta do usuario", async ({ page }) => {
  ...
  const pageInput = page.getByRole("spinbutton", { name: "Página" });
  await pageInput.fill("5");
  await page.getByRole("button", { name: "Salvar progresso" }).click();
  await expect(page.getByText("Progresso de leitura salvo.")).toBeVisible();
  await expect(
    page.locator(".stat-box").filter({
      has: page.getByText("última página salva"),
    })
  ).toContainText("5");
