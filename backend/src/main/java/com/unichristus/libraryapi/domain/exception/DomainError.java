...
public enum DomainError {
    ...
    USER_ROLE_SELF_CHANGE_FORBIDDEN("USER_ROLE_SELF_CHANGE_FORBIDDEN", "Não é permitido rebaixar o próprio acesso administrativo."),
    EMAIL_ALREADY_EXISTS("EMAIL_ALREADY_EXISTS", "Já existe um usuário cadastrado com o email: %s"),
    PASSWORD_RESET_TOKEN_INVALID("PASSWORD_RESET_TOKEN_INVALID", "Token de recuperação de senha inválido ou expirado."),
    ...
    // TAG ERRORS
    TAG_NOT_FOUND("TAG_NOT_FOUND", "Tag não encontrada: %s"),
    TAG_ALREADY_EXISTS("TAG_ALREADY_EXISTS", "Já existe uma tag cadastrada com o nome: %s"),

    // SEARCH ERRORS
    SEARCH_FILTER_INVALID("SEARCH_FILTER_INVALID", "Parâmetros de busca inválidos: %s"),

    // COLLECTION ERRORS
    COLLECTION_NOT_FOUND("COLLECTION_NOT_FOUND", "Coleção não encontrada: %s");
